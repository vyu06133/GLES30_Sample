...
public:
	...
	/// 
	/// </summary>
	/// <param name="activeTexture">glActiveTexture�Ńe�N�X�`�����j�b�g��I������悤�ɓ��삳����</param>
	Texture2D(GLuint activeTexture) : activeTexture_(activeTexture) {}
	
	/// <summary>
	/// activeTexture�I�v�V���������̏ꍇ�J�����g�e�N�X�`�����j�b�g���g��
	/// </summary>
	Texture2D() {}
	...
	bool TexImage(GLenum format = GL_RGBA, int width = 2, int height = 2, GLenum imageformat = GL_RGBA, void* image = nullptr)
	{
		width_ = width;
		height_ = height;
		format_ = format;
...
//		else
		{
			std::vector<uint8_t> temp(width_ * height_ * GetByteCount(imageformat), 0);
//			ZeroMemory(temp.data(), width_ * height_ * GetByteCount(imageformat));
	...
	bool TexStorage(GLenum format = GL_RGBA, int width = 2, int height = 2)
	{
	...
	bool TexSubImage(const Rect& rect, GLenum imageformat, void* image)
	{
		...
		assert(rect.left < width_);
		assert(rect.right < width_);
		assert(rect.top < height_);
		assert(rect.bottom < height_);
		glBindTexture(GL_TEXTURE_2D, id_);
		glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
